...
import ims.chat.ui.view.SwipeLayoutConv
...
class ConversationListAdapter(
    ...
    private val mDatas: MutableList<Conversation>?,
    private val mConversationListView: ConversationListView
) : BaseAdapter() {
    ...
    fun setToTop(conv: Conversation) {
        var oldCount = 0
        var newCount = 0
        ThreadUtil.runInUiThread { mConversationListView.setNullConversation(true) }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val headIcon = ViewHolder.get<ImageView>(convertView!!, R.id.msg_item_head_icon)
        val convName = ViewHolder.get<TextView>(convertView, R.id.conv_item_name)
        val content = ViewHolder.get<TextView>(convertView, R.id.msg_item_content)
        val datetime = ViewHolder.get<TextView>(convertView, R.id.msg_item_date)
        val newGroupMsgNumber = ViewHolder.get<TextView>(convertView, R.id.new_group_msg_number)
        val newMsgNumber = ViewHolder.get<TextView>(convertView, R.id.new_msg_number)
        val groupBlocked = ViewHolder.get<ImageView>(convertView, R.id.iv_groupBlocked)
        val newMsgDisturb = ViewHolder.get<ImageView>(convertView, R.id.new_group_msg_disturb)
        val newGroupMsgDisturb = ViewHolder.get<ImageView>(convertView, R.id.new_msg_disturb)

        val swipeLayout = ViewHolder.get<SwipeLayoutConv>(convertView, R.id.swp_layout)
        val delete = ViewHolder.get<TextView>(convertView, R.id.tv_delete)
        ...
        if (TextUtils.isEmpty(draft)) {
            ...
            if (lastMsg != null) {
                ...
                if (mAtAll.get(position) && ImsApplication.isAtall[gid] != null && ImsApplication.isAtall[gid]!!) {
                ...
                } else if (mArray.get(position) && ImsApplication.isAtMe[gid] != null && ImsApplication.isAtMe[gid]!!) {
                    //有人@我 文字提示
                    ...
                    if (lastMsg.targetType == ConversationType.group && contentStr != "[群成员变动]") {
                        ...
                        if (ImsApplication.isAtall[gid] != null && ImsApplication.isAtall[gid]!!) {
                            content.text = "[@所有人] $fromName: $contentStr"
                        } else if (ImsApplication.isAtMe[gid] != null && ImsApplication.isAtMe[gid]!!) {
                            content.text = "[有人@我] $fromName: $contentStr"
                            ...
                            content.text = "$fromName: $contentStr"
                        }
                    } else {
                        if (ImsApplication.isAtall[gid] != null && ImsApplication.isAtall[gid]!!) {
                            content.text = "[@所有人] $contentStr"
                        } else if (ImsApplication.isAtMe[gid] != null && ImsApplication.isAtMe[gid]!!) {
                            content.text = "[有人@我] $contentStr"
                        } else {
        ...
        if (convItem.type == ConversationType.single) {
            groupBlocked.visibility = View.GONE
            convName.text = convItem.title
            ...
            if (mGroupInfo != null) {
                ...
                if (blocked == 1) {
                    groupBlocked.visibility = View.VISIBLE
                } else {
                    groupBlocked.visibility = View.GONE
                }
            }
        ...
        if (convItem.unReadMsgCnt > 0) {
            newGroupMsgDisturb.visibility = View.GONE
            newMsgDisturb.visibility = View.GONE
            newGroupMsgNumber.visibility = View.GONE
            newMsgNumber.visibility = View.GONE
            if (convItem.type == ConversationType.single) {
                if (mUserInfo != null && mUserInfo!!.noDisturb == 1) {
                    newMsgDisturb.visibility = View.VISIBLE
                } else {
                    newMsgNumber.visibility = View.VISIBLE
                }
                if (convItem.unReadMsgCnt < 100) {
                    newMsgNumber.text = convItem.unReadMsgCnt.toString()
                } else {
                    newMsgNumber.text = "99+"
                }
            } else {
                if (mGroupInfo != null && mGroupInfo!!.noDisturb == 1) {
                    newGroupMsgDisturb.visibility = View.VISIBLE
                } else {
                    newGroupMsgNumber.visibility = View.VISIBLE
                }
                if (convItem.unReadMsgCnt < 100) {
                    newGroupMsgNumber.text = convItem.unReadMsgCnt.toString()
                } else {
                    newGroupMsgNumber.text = "99+"
                }
            }
    ...
    fun getDraft(convId: String): String {
        return mDraftMap[convId].toString()
    }
