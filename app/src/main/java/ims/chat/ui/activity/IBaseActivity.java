...
import ims.chat.utils.swipeback.app.SwipeBackActivity;
    ...
    public void onEventMainThread(LoginStateChangeEvent event) {
        ...
        switch (reason) {
            case user_logout:
                    ...
                    public void onClick(View v) {
                        switch (v.getId()) {
                            case R.id.jmui_cancel_btn:
                                Intent intent = new Intent(IBaseActivity.this, LoginActivity.class);
                                startActivity(intent);
                                break;
                            case R.id.jmui_commit_btn:
                                JMessageClient.login(SharePreferenceManager.getCachedUsername(), SharePreferenceManager.getCachedPsw(), new BasicCallback() {
                                    ...
                                    public void gotResult(int responseCode, String responseMessage) {
                                        if (responseCode == 0) {
                                            Intent intent = new Intent(IBaseActivity.this, MainActivity.class);
                                            startActivity(intent);
                                        }
                                    }
                                });
                ...
                dialog = DialogCreator.createLogoutStatusDialog(IBaseActivity.this, "您的账号在其他设备上登陆", listener);
