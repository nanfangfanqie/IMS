...
public class FriendInfoActivity extends IBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTargetId = getIntent().getStringExtra(ImsApplication.TARGET_ID);
        mTargetAppKey = getIntent().getStringExtra(ImsApplication.TARGET_APP_KEY);
        ...
        if (mIsFromContact || mIsFromSearch || mFromGroup || mIsAddFriend) {
            ...
            mGroupId = getIntent().getLongExtra(ImsApplication.GROUP_ID, 0);
            Conversation conv;
            if (mGroupId == 0) {
    ...
    public void startChatActivity() {
        if (mIsFromContact || mIsAddFriend || mIsFromSearch || mFromGroup) {
            ...
            intent.putExtra(ImsApplication.TARGET_ID, mUserInfo.getUserName());
            intent.putExtra(ImsApplication.TARGET_APP_KEY, mUserInfo.getAppKey());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == ImsApplication.RESULT_CODE_EDIT_NOTENAME) {
            ...
            FriendEntry friend = FriendEntry.getFriend(ImsApplication.getUserEntry(), mTargetId, mTargetAppKey);
            if (null != friend) {
